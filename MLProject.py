...
def page1_dataset():
    st.title("📦 Dataset")
    st.caption("Explore the customer churn dataset with rich EDA.")

    # Automatically load the dataset
    try:
        # Load the pre-uploaded CSV file
        df = pd.read_csv("Telco-Customer-Churn.csv")  # Change filename if needed
        ...
        # --- EDA Widgets (keep all the existing EDA code below) ---
        ...
        # ... (keep all the rest of your EDA code unchanged)
        
    except FileNotFoundError:
        st.error("❌ Error: 'customer_churn.csv' not found. Please ensure the file is in the same directory as the app.")
...
def page_nav():
    ...
    if st.session_state.show_home:
        ...
        bg_image_path = Path(__file__).parent / "Images.jpg"
